...
public class HiveReader {
    ...
    public static class Task extends Reader.Task {
        ...
        public void prepare() {
            ...
            if (!this.hive_sql_set.trim().equals(":;")){
                str.append(this.hive_sql_set);
            }
            String hiveQueryCmd = " use "+this.tempDatabase+"; create table "
            		+ this.tableName + " ROW FORMAT DELIMITED FIELDS TERMINATED BY '" +  this.hive_fieldDelimiter 
            		+"' STORED AS TEXTFILE "
                    +"location  '"+ tmpPath +"'"
            		+ " as " + this.hiveSql;
            str.append(hiveQueryCmd);


            LOG.info("hiveCmd ----> :" + str.toString());
            String cmd = "su hdfs -c " +"\"" + str.toString();


            //执行脚本,创建临时表
            if (!ShellUtil.exec(new String[]{this.hive_cmd, "-e", "\"" + cmd + " \""})) {
                throw DataXException.asDataXException(
